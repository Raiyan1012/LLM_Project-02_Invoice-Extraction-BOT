...
def create_docs(user_pdf_list):
    ...
    for filename in user_pdf_list:
        ...
        if match:
            ...
            try:
                # Attempt to convert the extracted text to a dictionary
                data_dict = eval('{' + extracted_text + '}')
            except SyntaxError as e:
                print(f"Error converting extracted text to dictionary: {e}")
                # Handle the error as appropriate for your application
                continue
            print(data_dict)
        else:
            print("No match found.")
            # Handle the case where no data is found, perhaps continue to next file
            continue

        # Only create a DataFrame from data_dict if it's not None
        if data_dict is not None:
            data_df = pd.DataFrame([data_dict])
            df = pd.concat([df, data_df], ignore_index=True)
            print("********************DONE***************")
        else:
            print("No data dictionary to add to DataFrame.")
